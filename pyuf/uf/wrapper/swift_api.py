...
class SwiftAPI():
    ...
    def close_conn(self):
        if 'swift' in self._nodes and self._nodes['swift']:
            self._nodes['swift'].close_conn()
    ...
    def set_rom_data(self, address, data, data_type=EEPROM_DATA_TYPE_BYTE):
        ...
        Returns:
            True on success
        '''
        ret = self._ports['service']['handle'].call(
            'set cmd_sync M2212 N1 A{} T{} V{}'.format(address, data_type, data))
        if ret.startswith('ok'):
            return True
    ...
    def shaker_module_init(self, wait=False, speed=4, time=5):
        '''
        init the shaker module. time is in seconds, speed unit is 1-4
